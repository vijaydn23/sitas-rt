...
export default function ChartPage() {
  ...
  const [form, setForm] = useState<Omit<ChartRow, 'id'>>({ ...blank });
  ...
  async function load() {
    const { data, error } = await supabase
      .from('exposure_chart')
      .select('*')
      .order('isotope', { ascending: true });

    if (error) setErr(error.message);
    else setRows((data ?? []) as ChartRow[]);
  }
  ...
  async function add() {
    setErr(null);
    const payload: Omit<ChartRow, 'id'> = {
    ...
    const { error } = await supabase.from('exposure_chart').insert([payload]);
    if (error) setErr(error.message);
    else {
      setForm({ ...blank });
      await load();
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-3 mb-3">
            <div>
              ...
              <select
                ...
                value={form.isotope}
                onChange={(e) =>
                  setForm((f) => ({ ...f, isotope: e.target.value as Isotope }))
                }
              >
            ...
            <div>
              ...
              <input
                className="border rounded p-2 w-full"
                value={form.film_speed}
                onChange={(e) => setForm((f) => ({ ...f, film_speed: e.target.value }))}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, thickness_min_mm: Number(e.target.value) }))
                }
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, thickness_max_mm: Number(e.target.value) }))
                }
              />
            </div>
            ...
            <div>
              ...
              <input
                type="number"
                className="border rounded p-2 w-full"
                value={form.base_constant}
                onChange={(e) =>
                  setForm((f) => ({ ...f, base_constant: Number(e.target.value) }))
                }
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="border rounded p-2 w-full"
                value={form.alpha}
                onChange={(e) => setForm((f) => ({ ...f, alpha: Number(e.target.value) }))}
              />
            </div>
